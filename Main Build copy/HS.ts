...
function drawVignette() {
    ...
    //draw darkness
    ctx.globalCompositeOperation = "source-over";// back to standart blending mode
...
function drawBlindFood() {
    ...
    flash1.onload = blind2.onload = blind3.onload = onImageLoad;
  
    function animateBlindFood() {
      // ... (rest of the animateBlindFood function remains the same)
    }
  }
...
//drawBackground();
ctx.fillStyle = "rgb(0, 0, 0)";
let blackrect: Path2D = new Path2D;
blackrect.rect(0,0,canvas.width, canvas.height);
ctx.fill(blackrect);
drawBounds();
drawWalls();
